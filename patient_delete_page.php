...
<script>
function delinfo() {
			del.action="del_info_patient.php?Email=<?php echo $em ?>";
			del.submit();
	}
function cancel() {
	del.action ="user_patient_detail.php?Email=<?php echo $em ?>";
	del.submit();
	}
</script>
</head>
...
<form name="del" action="" method="get">
 ...
 $p=$_SESSION['u_name'];
 include('include/DataConfig.php');
 $query = mysql_query("select * from p_reg where Email='$em'");
 ...
 <tr >
   ...
   <tr >
    ...
    <td ><input type="text" name="fname" value="<?php echo $f; ?>"  style="width:200px; border-radius:2px; background-color:#F8F8F8;"  /></td>
  </tr>
   <tr >
